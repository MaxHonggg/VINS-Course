...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM) {
        if (IsPoseVertex(vertex)) {//判断v->TypeInfo()是否为 “VertexPose”字符串
...
void Problem::AddOrderingSLAM(std::shared_ptr<myslam::backend::Vertex> v)
 {
    if (IsPoseVertex(v)) 
    {
        ...
        ordering_poses_ += v->LocalDimension();//随着顶点的加入，ordering_poses_也增加，从而记录了当前顶点在problem中的位置信息，从而便于于寻找雅可比对应块
    } 
    else 
...
vector<shared_ptr<Edge>> Problem::GetConnectedEdges(std::shared_ptr<Vertex> vertex) {
    ...
    auto range = vertexToEdge_.equal_range(vertex->Id());/*vertexToEdge_为unordered_multimap , 作用为由vertex id查询edge
    equal_range 返回范围[first,last)内等于指定值val的子范围的迭代器。
    注意的是使用这个函数的前提是范围[first,last)内的元素是有序的。
    同时注意函数的返回值类型，返回值是个pair对象，pair的first是左边界的迭代器，pair的second是右边界的迭代器*/
    for (auto iter = range.first; iter != range.second; ++iter) {

        // 并且这个edge还需要存在，而不是已经被remove了
        if (edges_.find(iter->second->Id()) == edges_.end())//在所有边中不存在带有该ID的边，说明被remove掉了？？？？
            continue;
...
bool Problem::RemoveVertex(std::shared_ptr<Vertex> vertex) {
    ...
    vector<shared_ptr<Edge>> remove_edges = GetConnectedEdges(vertex);//获得该顶点对应的 edge
    for (size_t i = 0; i < remove_edges.size(); i++) {
        RemoveEdge(remove_edges[i]);            // remove 该顶点对应的 edge.
    }

    if (IsPoseVertex(vertex))
        idx_pose_vertices_.erase(vertex->Id());     //从含维度的位姿顶点的map中删除该顶点，仍然以ID为索引
    else
        idx_landmark_vertices_.erase(vertex->Id());  //从含维度的路标点顶点的map中删除该顶点，仍然以ID为索引

    vertex->SetOrderingId(-1);          // used to debug，标记边ID为-1
    verticies_.erase(vertex->Id());     //从所有的顶点map中删除该顶点
    vertexToEdge_.erase(vertex->Id());   //从所有的 顶点-边 索引map中 删除该顶点
...
void Problem::SetOrdering() 
{
    ...
    for (auto vertex: verticies_) 
    {
        ...
        {
            AddOrderingSLAM(vertex.second);//向idx_landmark_vertices_与idx_pose_vertices_中添加顶点
        }
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM) 
    {
        ...
        for (auto landmarkVertex : idx_landmark_vertices_) 
        {
            ...
            );  //Again:OrderingId带有维度信息，表明了在雅可比矩阵中的位置
        }
    }
...
//#endif
    for (auto &edge: edges_) 
    {
        ...
        edge.second->ComputeResidual();     //边：计算残差，后续着重关注
        edge.second->ComputeJacobians();    //边：计算雅可比矩阵，后续着重关注
