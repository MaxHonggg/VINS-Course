...
{
    if (!init_feature)      //bool init_feature = 0;
    {
    ...
    if (first_image_flag)```// bool first_image_flag = true;
    {
    ...
    if (dStampSec - last_image_time > 1.0 || dStampSec < last_image_time)//间隔过大或者顺序错
    {
    ...
    last_image_time = dStampSec;            //给上一时刻时间戳赋值
    // frequency control
    ...
    const Eigen::Vector3d &vAcc)
{
    ...
    if (dStampSec <= last_imu_t)        // double last_imu_t = 0;
    {
